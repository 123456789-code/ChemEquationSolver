...
class Equation:
    ...
    def solve(self):
        ...
        if flag and self.coefficient != ["Infinity_solution"] and self.coefficient != ["No_solution"]:
            lcm = 1
            for i in self.coefficient:
                lcm = lcm * i.q // i.gcd(lcm, i.q)
            lcm_ = RationalNumber(-lcm)  # 设最后一值为1，导致调整等式方向的需求，此处lcm取相反数
